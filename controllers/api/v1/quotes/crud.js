...
const create = async (req, res, callback) => {
    try {
        ...
        const error = new Error('Create Quote failed');
        throw error;
    } catch (error) {
        console.log(error);
        callback('', error);
...
const findAll = async (req, res, callback) => {
    try {
        ...
        const error = new Error('There is no Quotes data');
        throw error;
    } catch (error) {
        console.log(error);
        callback('', error);
...
const findOne = async (req, res, callback) => {
    try {
        ...
        const doc = await db.Quote.findOne(options);
...
const update = async (req, res, callback) => {
    try {
        ...
        if (data.author_name) doc.author_name = data.author_name;
        if (imagePath){
            if (doc.image_url) {
...
const destroy = async (req, res, callback) => {
    try {
        ...
        const doc = await db.Quote.findByPk(id);
