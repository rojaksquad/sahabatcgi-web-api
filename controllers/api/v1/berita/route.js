...
router.get('/:id', async (req, res, next) => {
    await query.findOne(req, res, (data, error) => {
        if(!error){
            ...
            Responser.error(res, "Error Get Berita by ID: " + error.message, error.result || error, error.code || 400);
        }
    })
});
...
router.patch('/:id', jwtAuth, upload.single('image'), Validator.update, async (req, res, next) => {
    ...
    await query.update(req, res, (data, error) => {
        if(!error){
            ...
            Responser.error(res, "Error Update Berita by ID: " + error.message, error.result || error, error.code || 400);
        }
    })
});
